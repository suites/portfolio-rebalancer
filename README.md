...
> 현재 상태: 토스증권 실제 계좌·보유자산과 통화별 매수 가능 금액을 Prisma/PostgreSQL 불변 스냅샷으로 저장하고, 6개 운영 화면에서 조회하는 shadow 수직 슬라이스가 구현되었습니다. 현재 보유종목을 `안전자산 / 핵심 공격자산 / 위성 공격자산`으로 분류하고 `관리 현금`을 더한 네 자산군 목표를 버전형 초안으로 저장·적용할 수 있습니다. 자산군 내부 종목 비중은 현재 평가액 비율을 largest-remainder 방식으로 정확히 10000점에 고정하며, 새 read-only 수집에서 같은 활성 버전의 정책·주식 평가액·관리 현금·총액을 함께 저장합니다. 매수 가능 금액은 관리 현금과 분리된 주문 가능성 참고 증거로만 유지합니다. 주문 계획, 주문 원장과 paper/live 실행은 아직 구현되지 않아 계속 차단되며 실거래 쓰기 전송도 코드에서 하드 차단됩니다.
