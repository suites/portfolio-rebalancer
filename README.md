...
> 현재 상태: 토스증권 실제 계좌·보유자산을 Prisma/PostgreSQL 불변 스냅샷으로 저장하고, 6개 운영 화면에서 조회하는 shadow 수직 슬라이스가 구현되었습니다. 현재 보유종목의 목표 비중은 버전형 초안으로 저장하고 별도로 적용할 수 있으며, 새 read-only 수집에서 해당 버전을 스냅샷에 고정합니다. 검증된 관리 현금, 주문 계획, 주문 원장과 paper/live 실행은 아직 구현되지 않아 계속 차단됩니다. 실거래 쓰기 전송도 코드에서 하드 차단됩니다.
