...
> 현재 상태: 토스증권 실제 계좌·보유자산과 통화별 매수 가능 금액을 Prisma/PostgreSQL 불변 스냅샷으로 저장하고, 6개 운영 화면에서 조회하는 shadow 수직 슬라이스가 구현되었습니다. 현재 보유종목과 `CASH`의 목표 비중, 현금 제외 또는 사용자 고정 원화 관리금액을 버전형 초안으로 저장하고 별도로 적용할 수 있으며, 새 read-only 수집에서 같은 활성 버전의 정책·주식 평가액·관리 현금·총액을 함께 고정합니다. 매수 가능 금액은 관리 현금과 분리된 주문 가능성 참고 증거로만 저장합니다. 주문 계획, 주문 원장과 paper/live 실행은 아직 구현되지 않아 계속 차단되며 실거래 쓰기 전송도 코드에서 하드 차단됩니다.
