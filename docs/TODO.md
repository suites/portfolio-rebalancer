...
체크 표시는 현재 저장소에서 자동 테스트로 검증한 코드·문서 범위를 뜻합니다. 실제
계좌 주문, 장기 관찰, 사용자 검증과 운영 런북처럼 외부 시간이나 별도 승인이 필요한
항목은 코드 경로가 구현되어도 별도 체크하지 않습니다.
...
- [x] 주문·차단·복구의 한국어 결과 상태와 보호 조치·다음 행동 형식 정의
...
- [x] `EQUAL_V1` largest-remainder 내부 균등 배분과 미보유 종목의 명시적 정책 요구
...
- [x] 설정 버전 단위 종목 중복 배정 UNIQUE와 부모 버전 복합 FK
- [x] append-only 종목 검증 증거, mutable 로컬 카탈로그 최신 포인터와 목표 종목 검증 FK
- [x] 전역 Toss 수집 lease와 fencing token 구현
- [x] 수집 lease heartbeat, 만료 takeover와 owner·fencing token 기반 안전 해제 구현
- [x] `logical_order_id` UNIQUE 제약 구현
- [x] Live 승인 묶음, pre-submit·원장·예약, A authorization을 각각 Serializable 원자 경계로 구현
- [x] 계획을 주문 제출과 분리해 먼저 불변 저장하고 plan hash로 실행 증거에 고정
- [x] append-only 주문 상태 이력 구현
- [x] A 이후 B가 없는 비전송 복구와 10분 `UNKNOWN_BLOCKED` 복구 테스트
- [ ] 실제 프로세스 강제 종료·재시작 후 계좌 전체 미완료 주문 자동 스캔 E2E
- [x] 로그와 저장 데이터의 비밀정보 마스킹 테스트
- [x] Web `status`·대사·취소·exact `recover`가 DB 직접 수정 없이 상태를 처리
- [ ] CLI `status`, `explain`, `recover` 구현
- [x] migration owner와 최소권한 `DATABASE_RUNTIME_URL` 역할 분리
- [x] runtime 역할의 DDL·TRUNCATE·trigger 우회·migration ledger 접근 차단 통합 테스트
...
- [x] 포트폴리오 계획에 필요한 토스 원본 응답을 증권사 중립 모델로 변환하는 어댑터 구현
- [ ] 랭킹·지표·캔들 등 비핵심 조회 API의 중립 어댑터 구현
- [x] 고정 OpenAPI 재생 parity와 전체 검증을 GitHub Actions CI에 연결
...
- [x] 계좌 및 보유 주식 조회
- [x] 가격, 호가, 가격 제한, 종목과 시장 캘린더 조회
- [x] 서버 종목명 검색 카탈로그와 Toss 정확 심볼·유의사항 검증을 분리한 종목 추가 API
...
- [x] 수수료 조회
- [x] 목표 종목 검증용 종목 경고 조회와 현재 거래 차단 판정
- [x] 계좌·보유·환율 응답 스키마 검증
- [x] rate-limit과 request ID 응답 헤더 메타데이터 추출
- [x] 그룹별 client-side limiter 구현
- [x] GET 429의 제한된 `Retry-After` 백오프와 jitter 구현
- [x] timeout, 네트워크, 4xx와 5xx 안전 오류 분류
- [x] request ID와 rate-limit metadata 감사 증거 저장
...
- [x] 데이터 freshness 규칙 적용
- [x] 설정 외 자산 탐지 및 차단
- [x] 주문 없이 불변 리밸런싱 계획 생성
- [ ] 사람이 계산한 표본과 결과 대조
- [ ] 최소 2주간 일일 shadow 실행 관찰
- [x] `NO_ACTION`과 차단 이유 저장·화면 표시
- [ ] 종목·비중 변경 전 config diff와 주문 영향 preview 구현
- [x] 한국어 실행·차단·보호 조치 문구 구현
- [ ] 초보 사용자가 이해할 수 있는 실행 결과 사용성 검증

## Phase 5 — 위험 차단기

- [x] append-only 수동 킬 스위치 구현
- [x] 일일 총거래금액과 회전율 제한
- [x] 단일 주문금액 제한
- [x] 종목·자산군·전체 위험자산 최대 비중 제한
- [x] 미체결 및 `UNKNOWN` 주문 차단
- [x] 가격 급변과 stale quote 차단
- [x] 장 상태와 거래 제한 차단
- [x] 반대 방향 주문 충돌 차단
- [x] 매수 가능 금액·매도 가능 수량·수수료·상하한가 사전 검증
- [x] fail-closed 단위·통합 테스트

## Phase 6 — Paper 실행기

- [x] `OrderExecutor` 인터페이스 정의
- [x] 결정적 `clientOrderId` 생성
- [ ] 시장가 슬리피지 모델 구현
- [x] 한국 지정가 `DAY` 체결 규칙 구현
- [x] 수수료 모델 구현
- [x] 호가잔량 10% 참여 기반 부분체결 시뮬레이션 구현
- [x] 주문 상태 머신 구현
- [x] 브로커 원본 상태와 내부 정규화 상태 분리
- [x] 10분 멱등성 경계 9:59/10:01 테스트
- [x] `clientOrderId` 36자 및 허용 문자 제약 테스트
- [x] 10분 이후 `UNKNOWN_BLOCKED` 자동 재제출 금지 테스트
- [ ] 매도 체결 후 매수 계획 재계산
- [x] 같은 실행의 Phase A/Phase B 계획 버전 DB 불변식 구현
- [ ] 체결 후 새 snapshot을 수집해 Phase B를 자동 생성하는 saga
- [x] 네트워크 오류와 모호한 응답 시나리오 모의 테스트
...
아래 항목은 코드 안전장치와 실제 운영 승격을 구분합니다. 코드 항목이 완료되어도
실계좌 검증·장기 비교·독립 검토·런북이 완료되기 전에는 운영 승격 완료로 간주하지
않습니다.

- [ ] paper와 shadow 결과의 장기 비교
- [x] 부분체결, 취소, timeout과 `UNKNOWN_BLOCKED` 복구 코드·테스트
- [ ] 주문 정정 지원과 실계좌 동작 검증
- [x] 중복 클릭·동일 logical order·A/B dispatch 중복 차단 테스트
- [ ] 실제 프로세스 강제 종료 E2E
- [x] 계좌 허용 목록과 다중 활성화 조건 설계·구현
- [x] 주문별·일별 극소액 한도 설계·구현
- [x] 만료되는 주문별 수동 승인 흐름 설계·구현
- [x] raw 쓰기 API 하드 차단과 승인 전용 `TossLiveOrderAdapter` 분리
- [x] Live 한 실행의 첫 주문 한 건만 일회성 dispatch하고 즉시 대사
- [x] A 이후 B가 없는 경우에만 가능한 비전송 증거 복구
- [x] B 직전 계좌 잠금 아래 최신 ACTIVE 설정·승격·킬 스위치와 계좌 HMAC 증거 재검증
- [x] B 이후 응답 저장 전 중단을 자동 주문 귀속 없이 no-ID `UNKNOWN_BLOCKED`와 exact 운영자 복구로 처리
- [x] A 이전 `PLANNED` 고착을 브로커 증거 없음이 증명된 경우에만 `REJECTED`로 복구
- [ ] 장애 대응과 주문 정합성 런북
- [ ] 세금, 환전 및 계좌 유형 제약 확인
- [x] 실거래 코드 독립 리뷰와 `docs/LIVE_SAFETY_REVIEW.md` 기록
- [ ] 한 종목·극소액·수동 승인 실거래 검증
- [ ] 실계좌 결과와 주문 원장 대사
- [x] 운영자가 Web 원장·대사·취소·exact recover로 DB 수정 없이 대응
- [ ] `status`, `explain`, `recover` 전체 운영 흐름과 사용성 검증

## 문서 백로그

- [x] 기술 스택과 합성 데이터 Web GUI 로컬 개발 가이드 작성
- [x] 설정 레퍼런스 작성
- [x] 토스증권 어댑터 오류·재시도·쓰기 비재시도 매핑 문서 작성
- [ ] 데이터 모델과 ER 다이어그램 추가
- [x] paper 체결 모델의 지정가·호가잔량·수수료 가정과 한계 문서화
- [ ] 실거래 검토 시 보안 위협 모델 작성

## Web GUI 구현 트랙

### UI-1.2 — 초보자용 포트폴리오 만들기

- [x] 주 메뉴를 홈·포트폴리오 만들기·리밸런싱 세 경로로 축소
- [x] 안정형·균형형·성장형 투자성향만으로 승인된 ETF 모델과 비중 제안
- [x] 추천 종목을 7개 이하로 제한하고 추천 밖 보유종목을 목표 0% 정리 후보로 보존
- [x] 포트폴리오 적용 한 번으로 내부 저장·활성화·최신 수집·예상 주문 계산을 연결하고 주문 실행 전 최종 확인 유지
- [x] 종목 검색·자산군·관리 현금·운영 한도·Live 승격을 고급 설정으로 이동
- [x] 모든 화면에서 접근 가능한 상단 우측 정보 새로고침 제공
- [x] 상단 운영 상태 배지를 안전 조건을 유지하는 실거래 ON/OFF 스위치로 단순화
- [ ] 모델 유니버스 버전과 변경 승인 이력을 DB에 append-only로 저장
- [ ] AI 후보 정렬·설명 도입 전 독립 안전성·적합성 검토
...
- [x] 5. 별도 `/troubleshooting` 화면을 제거하고 홈 차단 상태와 전역 정보 새로고침으로 통합
- [x] 6. `/settings` 버전형 목표 비중 조회·검증·저장과 새 스냅샷 요구 상태 구현
- [x] 7. 5개 SSR 라우트·계약·설정 안전성·접근성 구조·반응형 CSS 자동 검증과 문서 동기화
- [x] 8. 모바일 최상위 섹션 간격 통일과 내부 구현 설명의 제품용 문구 정리

### UI-2 — 읽기 전용 운영 화면

- [x] GUI가 공통 engine 애플리케이션 서비스를 사용하는 BFF/API 경계 구현
- [ ] CLI를 같은 애플리케이션 서비스에 연결
- [x] 실제 토스 서버 스냅샷을 공통 애플리케이션 서비스와 Zod 계약으로 전달
- [x] 홈 상태 요약과 최근 활동
- [x] 홈에서 주문 없이 최신 토스 자산을 다시 수집하고 실패 시 홈에 차단 상태 표시
- [x] 포트폴리오 `AllocationBand`와 텍스트 대체 설명
- [x] 포트폴리오 접근 가능한 표
- [x] 보유자산 계층과 관리되지 않는 자산 표시
- [x] 리밸런싱 이유와 현재/목표 비교
- [ ] 주문 후 예상 비중 Before/After 비교
- [x] 주문 없는 위험 검사 결과 표시
...
- [x] 현재 보유종목의 `SAFE/CORE/SATELLITE` 분류와 자산군 목표 입력
- [x] 현재 미보유 종목 검색·검증·자산군 추가 UI
- [ ] 설정 변경 Diff와 거래 영향 미리보기
- [x] 저장된 주문 계획 표와 기본 점검 항목
- [ ] 주문별 전체 Risk Gate 증거 체크리스트
- [ ] 설정 변경 시 기존 계획 자동 무효화

### UI-4 — 실행·기록·복구

- [x] Paper·Live 구분과 금융 거래 최종 확인
- [x] 실제 주문 원장과 append-only 상태 타임라인
- [ ] 전용 실행 영수증과 입력·위험 증거 상세
- [x] 부분체결·거부·취소·상태 불명 UI
- [ ] 감사 로그와 입력 스냅샷 상세
- [x] 주문 대사·취소·`UNKNOWN_BLOCKED` exact recover 화면
- [x] 킬 스위치 활성화·사유 입력 해제와 Live 승격 화면
- [ ] 킬 스위치 해제 직전 원인·위험 재검사 UX
...
- [x] Tailscale 내부망·Caddy·engine loopback 접근 경계 검증
- [ ] CSP 정책과 외부 스크립트 경계 검증
- [x] 서버 단위·통합 테스트에서 중복 클릭·동일 논리 주문·네트워크 재시도 차단
- [ ] 새로고침·뒤로 가기를 포함한 브라우저 중복 주문 E2E
- [x] 계획 hash·설정 버전·새 스냅샷 불일치의 오래된 실행 차단 테스트
- [x] `UNKNOWN`·부분체결·기존 미체결 상태의 신규 주문 및 수동 재제출 차단 테스트
