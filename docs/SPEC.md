...
현재 구현은 실제 조회 기반 shadow 수직 슬라이스입니다. 토스 계좌·보유자산, KRW·USD 매수 가능 금액과 필요한 USD/KRW 환율을 런타임 검증하고, Prisma/PostgreSQL 불변 스냅샷으로 저장한 뒤 별도 engine 계약을 통해 6개 Web GUI 화면에 표시합니다. 매수 가능 금액은 `valuationEligible=false`인 별도 증거로 저장하며 평가용 관리 현금이나 총 포트폴리오 가치에 자동 합산하지 않습니다. 사용자는 관리 현금을 평가에서 제외하거나 고정 원화 금액으로 명시하고, 모든 현재 보유종목을 `SAFE`, `CORE`, `SATELLITE` 중 정확히 한 자산군에 배치합니다. `CASH`를 포함한 네 자산군의 목표·허용 범위와 구성 종목은 원본 스냅샷 ID·digest에 묶인 DRAFT 버전 저장과 별도 ACTIVE 전환으로 관리합니다. 자산군 내부 종목 비중은 현재 평가액 비율을 `PRESERVE_CURRENT_V1` largest-remainder 정책으로 고정합니다. 새 수집의 최종 fenced 트랜잭션은 고정할 ACTIVE 버전과 현금 정책을 한 번에 읽어 주식 평가액, 관리 현금과 총 관리 자산을 함께 저장합니다. 주문 계획, 주문 원장, paper 체결과 live 실행은 아직 구현 범위에 포함되지 않습니다.
...
현재 목표 설정 API는 `SAFE`, `CORE`, `SATELLITE`, `CASH`를 정확히 한 번씩 포함하고 목표 합이 `10000bp`인지 검증합니다. 최신 스냅샷의 모든 보유종목은 비현금 자산군 중 정확히 한 곳에 있어야 하며, 같은 종목의 복수 배정과 현재 미보유 종목은 이 단계에서 거부합니다. 종목이 없는 비현금 자산군은 목표 0%만 허용하고 `CASH`에는 종목을 포함할 수 없습니다. 현금 정책은 `EXCLUDED/CASH_V1` 또는 PostgreSQL signed bigint 범위의 정규화된 원 단위 문자열을 갖는 `FIXED_KRW/CASH_V1`이어야 하며, 제외 정책의 `CASH` 목표는 0%여야 합니다. 기본 입력은 목표와 `AUTO/MIXED_V1` 정책만 받으며 engine이 확정 범위를 계산합니다. 고급 `CUSTOM_V1` 입력과 저장 결과는 모두 `0 <= lower <= target <= upper <= 10000bp`를 만족해야 합니다. 클라이언트가 보낸 자산군 label, 종목명·시장·통화를 신뢰하지 않고 서버 정의와 최신 스냅샷에서 파생합니다. 초안 source와 content hash에는 원본 스냅샷 ID·digest, 현금 정책, 밴드 정책, 구성 정책, 확정 범위, 정렬된 구성 종목과 `withinAssetPoints`를 포함합니다. DB는 `(config_version_id, marketCountry, symbol)` UNIQUE와 부모 allocation 버전 복합 FK로 서비스 우회 쓰기도 차단합니다. 저장과 적용 트랜잭션은 DB의 최신 스냅샷을 다시 읽으며, 원본과 다르면 `DRAFT_STALE`로 아무 설정도 변경하지 않고 새 초안을 요구합니다. 이전 개별 종목 형식의 DRAFT는 새 자산군 초안으로 다시 만들기 전까지 적용하지 않습니다. 초안 저장은 기존 ACTIVE를 변경하지 않으며, 별도 적용 트랜잭션에서 이전 ACTIVE를 RETIRED로 전환합니다. 활성 버전과 최신 스냅샷의 고정 버전이 다르면 `TARGET_CONFIG_STALE`로 계획과 주문을 차단하고 새 read-only 수집을 요구합니다.
...
현재 shadow 수직 슬라이스는 사용자 지정 관리 현금과 목표 범위 판정까지 구현되어 있습니다. 브로커 현금 source of truth 검증, pretrade 검사와 원장이 없으므로 주문 계획은 아직 생성하지 않습니다.
