...
현재 Web GUI는 `/`, `/portfolio`, `/rebalancing`, `/orders`, `/settings`를
제공합니다. 스냅샷과 설정에 고정된
...
Web에서 사용자가 관리하는 실거래 인터페이스는 전역 `실거래 ON/OFF` 스위치 하나뿐입니다.
내부 상태는 별도 사용자 조작 기능으로 노출하지 않고 ON 요청 시 서버가 자동 검증하며,
검증 실패는 OFF 유지와 주문 차단으로 처리합니다.
추천 포트폴리오의 기본 동작은 한 번의 `포트폴리오 적용하기` 요청으로 내부 목표 버전 저장과
활성화, 최신 read-only 계좌 수집 및 현재 실거래 ON/OFF에 맞는 예상 주문 계산까지 수행합니다.
주문 제출은 이 요청에 포함하지 않고 계산된 종목·방향·수량·지정가·예상 금액을 보여 준 뒤
별도의 최종 실행 동작에서만 수행합니다.
브라우저는 승인 ID, 계좌 HMAC과 Toss 비밀정보를 받지 않습니다. CLI, 전체 Risk Gate
증거 뷰어와 자동 Phase B는 후속 범위입니다.
