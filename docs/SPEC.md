...
현재 구현은 실제 조회 기반 shadow 수직 슬라이스입니다. 토스 계좌·보유자산, KRW·USD 매수 가능 금액과 필요한 USD/KRW 환율을 런타임 검증하고, Prisma/PostgreSQL 불변 스냅샷으로 저장한 뒤 별도 engine 계약을 통해 6개 Web GUI 화면에 표시합니다. 매수 가능 금액은 `valuationEligible=false`인 별도 증거로 저장하며 평가용 관리 현금이나 총 포트폴리오 가치에 자동 합산하지 않습니다. 사용자는 관리 현금을 평가에서 제외하거나 고정 원화 금액으로 명시하고, 모든 현재 보유종목을 `SAFE`, `CORE`, `SATELLITE` 중 정확히 한 자산군에 배치합니다. 현재 미보유 종목은 이전 Toss 검증 결과로 구성한 로컬 카탈로그에서 검색하거나 국내 6자리 코드·미국 티커를 Toss 종목 기본정보와 유의사항으로 정확히 재검증한 뒤 목표 구성에 추가할 수 있습니다. `CASH`를 포함한 네 자산군의 목표·허용 범위와 구성 종목은 원본 스냅샷 ID·digest 및 미보유 종목의 검증 ID에 묶인 DRAFT 버전 저장과 별도 ACTIVE 전환으로 관리합니다. 자산군 내부 종목 비중은 현재 평가액 비율을 `PRESERVE_CURRENT_V1` largest-remainder 정책으로 고정하거나, 미보유 종목이 포함된 경우 사용자가 선택한 `EQUAL_V1`로 균등 확정합니다. 새 수집의 최종 fenced 트랜잭션은 고정할 ACTIVE 버전과 현금 정책을 한 번에 읽어 주식 평가액, 관리 현금과 총 관리 자산을 함께 저장합니다. 주문 계획, 주문 원장, paper 체결과 live 실행은 아직 구현 범위에 포함되지 않습니다.
...
현재 공식 API는 REST 기반입니다. 별도의 paper 또는 sandbox 주문 API가 확인되지 않았으므로 paper 체결은 애플리케이션 내부에서 구현합니다. 계좌·보유·매수 가능 금액·USD/KRW 환율·종목 기본정보·종목 유의사항 응답은 Zod 런타임 스키마로 검증합니다. 계좌 수집 응답은 redaction 후 저장하고, 종목 검증은 원본 payload의 정규화 JSON과 SHA-256을 append-only 증거로 저장합니다. 나머지 조회 operation의 중립 모델 변환과 대사는 후속 구현입니다.
...
현재 shadow 수직 슬라이스는 사용자 지정 관리 현금, 목표 범위 판정, 로컬 검증 카탈로그 검색, Toss 정확 심볼·유의사항 검증과 미보유 목표 종목 고정까지 구현되어 있습니다. 종목 검증의 `tradeBlockedNow`는 향후 계획·주문 직전 재검증 입력이지만, 브로커 현금 source of truth 검증, 전체 pretrade 검사와 원장이 없으므로 주문 계획은 아직 생성하지 않습니다.
